...
C***** SUBROUTINE *****
      SUBROUTINE DATIN (LAYCON,NCON,HEAD,DELT,XMAX,YMAX,DELR,DELC,DELZ,
...
C
     ...
     1,DELZCB(NLAY),XMAX(NCOL),YMAX(NROW),ZTOP(NZDIM),ZBOT(NZDIM),
     ...
     6ZLLC(NPART),QSS(NCOL,NROW,NLAY),WELSV(NWELSV),DRNSV(NDRNSV),
     7RIVSV(NRIVSV),GHBSV(NGHBSV),RECHSV(NCOL,NROW),IRCHSV(NCOL,NROW),
     8ITMPO(NTMPO),QSTO(NCOL,NROW,NLAY),INILOC(NPART),
     9PERLEN(NPER),NUMTS(NPER),TIMX(NPER),STRSV(NSTRSV),
...
C
C  READ STRESS PACKAGE HEADER RECORDS
      IF(IUNIT(1).GT.0) THEN
        READ(IUNIT(1),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,NRCHOP,RDUMMY,I7,IUNIT(1))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,IDUMMY,RDUMMY,I7,IUNIT(1))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,IRCHTP,RDUMMY,I7,IUNIT(1))
      END IF
      IF(IUNIT(2).GT.0) THEN
        READ(IUNIT(2),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,MXWELS,RDUMMY,I7,IUNIT(2))
      END IF
      IF(IUNIT(3).GT.0) THEN
        READ(IUNIT(3),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,NEVTOP,RDUMMY,I7,IUNIT(3))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,IDUMMY,RDUMMY,I7,IUNIT(3))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,IEVTTP,RDUMMY,I7,IUNIT(3))
      END IF
      IF(IUNIT(4).GT.0) THEN
        READ(IUNIT(4),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,MXRIVS,RDUMMY,I7,IUNIT(4))
      END IF
      IF(IUNIT(5).GT.0) THEN
        READ(IUNIT(5),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,MXDRNS,RDUMMY,I7,IUNIT(5))
      END IF
      IF(IUNIT(6).GT.0) THEN
        READ(IUNIT(6),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,MXGHBS,RDUMMY,I7,IUNIT(6))
      END IF
      IF(IUNIT(9).GT.0) THEN
        READ(IUNIT(9),'(A)') LINE
        ICOL=1
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,MXSTRS,RDUMMY,I7,IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,STRSV(NSTRSV-3),I7,
     1              IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,STRSV(NSTRSV-2),I7,
     1              IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,STRSV(NSTRSV-1),I7,
     1              IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,STRSV(NSTRSV),I7,
     1              IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,CONST,I7,IUNIT(9))
        CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,ICBSTR,RDUMMY,I7,IUNIT(9))
      END IF
C
...
20    NCON(N)=0
      ...
      IF(NCBL.EQ.0) THEN
      WRITE(I7,5000)
      WRITE(I7,*) 'NO CONFINING LAYERS.  NCON = 0 FOR ALL LAYERS.'
      ELSE
      WRITE(I7,5000)
      WRITE(I7,*) 'NCON (CONFINING LAYER CODES):'
      WRITE(I7,5080) (NCON(N),N=1,NLAY)
      END IF
      IF(NCBL.GT.0) THEN
      NN=0
      DO 30 N=1,NLAY
...
C
      ANAME= 'DELR'
      CALL U1DREL(DELR,ANAME,NCOL,IU,I7)
C
...
C
      ANAME= 'DELC'
      CALL U1DREL(DELC,ANAME,NROW,IU,I7)
C
...
C
      IF (IGRID.EQ.1) THEN
      ANAME= 'DELZ (Z SPACING)'
      CALL U1DREL(DELZ,ANAME,NLAY,IU,I7)
      IF(NCBL.GT.0) THEN
      ANAME= 'DELZCB (CB THICKNESS)'
      CALL U1DREL(DELZCB,ANAME,NLAY,IU,I7)
      ELSE
      DO 60 N=1,NLAY
60    DELZCB(N)=0.0
      END IF
      READ(IU,'(A)') LINE
      ICOL=1
      CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,ZBL1,I7,IU)
      ZBOT(1)=ZBL1
      WRITE(I7,5000)
...
70    CONTINUE
      ...
      CALL U2DREL(ZTOP(KP),ANAME,NROW,NCOL,K,IU,I7)
      END IF
      ANAME= 'ZBOT (BOTTOM ELEVATION)'
      CALL U2DREL(ZBOT(KP),ANAME,NROW,NCOL,K,IU,I7)
80    CONTINUE
      END IF
C
C  IBOUND DATA
...
5320  FORMAT(1X/1X,'TIME AT BEGINNING OF SIMULATION = ',E14.6)
      ...
      READ(IU,'(A)') LINE
      ICOL=1
      CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,PERLEN(N),I7,IU)
      CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,2,NUMTS(N),RDUMMY,I7,IU)
      CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,NDUMMY,TIMX(N),I7,IU)
      WRITE(I7,5340) N,PERLEN(N),NUMTS(N),TIMX(N)
...
C
     ...
     1            KKSTP,IERR)
      ...
      TIMREL=TBGABS
      CALL GETPS(PERLEN,NUMTS,TIMX,NPER,TIMREL,TBEGIN,KKPER,KKSTP,IERR)
      IF(IERR.GT.0) THEN
...
C
     ...
     1 DELC,WELSV,DRNSV,RIVSV,GHBSV,STRSV,RECHSV,IRCHSV,ITMPO,
