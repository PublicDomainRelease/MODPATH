...
C***** SUBROUTINE *****
      SUBROUTINE RDBUDG (BUFF,LABEL,NCOL,NROW,NLAY,IU,IO,LPER,
     1                   LSTP)
      DIMENSION BUFF(NCOL*NROW*NLAY)
      CHARACTER*16 LABEL,TEXT
C
C....IF CALLING ROUTINE PASSES VARIABLE "LABEL" WITH A BLANK VALUE, THEN THIS ROUTINE
C....READS THROUGH BUDGET FILE AND RETURNS THE NEXT SET OF DATA THAT MATCHES THE
C....STRESS PERIOD AND TIME STEP.
C
C....IF CALLING ROUTINE PASSES VARIABLE "LABEL" WITH A NON-BLANK VALUE, THEN THIS ROUTINE
C....READS THROUGH BUDGET FILE AND ONLY RETURNS WHEN IT FINDS THE DATA SET THAT MATCHES
C....THE STRESS PERIOD, TIME STEP, AND HEADER TEXT STRING (TEXT=LABEL).
C
C....IN BOTH CASES, THIS ROUTINE SETS THE VARIABLE "LABEL" EQUAL TO THE TEXT STRING
C....READ ON THE HEADER RECORD BEFORE RETURNING.
 
20    CONTINUE
C... READ HEADER RECORDS
      CALL GETBDH(IU,KSTP,KPER,TEXT,NC,NR,NL,NCB,DELT,PERTIM,TOTIM,
     1            NEWBUD,IEND)
C... CHECK TO SEE IF HEADER IS FOR A TIME STEP BEYOND THE REQUESTED STEP
C    IF SO, AND INITIAL LABEL IS BLANK, THEN BACKSPACE THE FILE ONE RECORD, SET
C    VALUES IN BUFF TO 0.0, AND SET THE VALUE OF LABEL TO BE "END DATA". THEN RETURN.
      IF(LABEL.EQ.' ') THEN
        IABORT=0
        IF(KPER.GT.LPER) IABORT=1
        IF(KPER.EQ.LPER .AND. KSTP.GT.LSTP) IABORT=1
        IF(IEND.EQ.1) IABORT=1
        IF(IABORT.EQ.1) THEN
          BACKSPACE (IU)
          LABEL='END DATA'
...
22        BUFF(N)=0.0
        ...
        ENDIF
      ENDIF
C... HANDLE RETURN CODES ( IF NORMAL, IEND=0 )
      IF(IEND.EQ.1) THEN
      GO TO 30
...
C... WRITE AN ERROR MESSAGE & QUIT IF DIMENSIONS ARE INCONSISTENT
      IF(NC.NE.NCOL .OR. NR.NE.NROW .OR. NL.NE.NLAY) GO TO 55
 
C... READ THE BUDGET TERMS IN WHATEVER FORM THEY WERE SAVED & RETURN
C    IN 3-D BUFF ARRAY
      CALL GETBDR(BUFF,NCOL,NROW,NLAY,NCB,IU)
 
C.....CHECK TO SEE IF THIS IS THE RIGHT DATA SET. IF SO, RETURN. OTHERWISE, TRY AGAIN
C
C.....FIRST MAKE SURE THE TIME STEP MATCHES...
      IF (LPER.EQ.KPER .AND. LSTP.EQ.KSTP) THEN
C.......TIME STEP MATCHES, NOW CHECK THE TEXT VARIABLE...
C.........IF LABEL IS BLANK, ACCEPT THE DATA, SET LABEL=TEXT, AND RETURN.
        IF(LABEL.EQ.' ') THEN
          LABEL=TEXT
          RETURN
C.........IF LABEL IS NOT BLANK, ACCEPT DATA AND RETURN ONLY IF LABEL=TEXT
        ELSE IF(LABEL.EQ.TEXT) THEN
          RETURN
        END IF
      END IF
C.....NONE OF THE CONDITIONS WERE MET, GO BACK, READ ANOTHER DATA SET & TRY AGAIN
      GO TO 20
...
5000  FORMAT(' UNIT ',I3,': END OF FILE ON MODFLOW BUDGET FILE.'/
     1  'BUDGET DATA INCOMPLETE. STOP')
      STOP
40    WRITE(IO,5100) IU
5100  FORMAT(' ERROR READING BUDGET HEADER RECORD ON UNIT ',I3,' STOP.')
      STOP
50    WRITE(IO,5300) IU
5300  FORMAT(' ERROR READING SECONDARY BUDGET HEADER RECORD ON UNIT ',
     1I3,' STOP.')
      STOP
55    WRITE(IO,5400) NR,NC,NL
5400  FORMAT(' INCONSISTENT GRID DIMENSIONS IN BUDGET HEADER RECORD.'
     1/' FILE CONTAINED (NROW,NCOL,NLAY) = (',I4,',',I4,',',I4,')'
     2/' STOP.')
      STOP
      END
 
C***** SUBROUTINE *****
      SUBROUTINE GETBDR(BUFF,NCOL,NROW,NLAY,NCB,IU)
      DIMENSION BUFF(NCOL*NROW*NLAY)
 
      NRC=NCOL*NROW
      NT=NRC*NLAY
 
C... READ THE BUDGET INFO FOR THE APPROPRIATE STORAGE ALGORITHM
 
C : NORMAL 3-D ARRAY
      IF(NCB.EQ.1) THEN
         READ(IU) (BUFF(I),I=1,NT)
         RETURN
      END IF
 
C... ZERO THE BUFF ARRAY BECAUSE THE FOLLOWING STORAGE SCHEMES READ IN
C    ONLY SOME OF THE ELEMENTS OF THE BUFF ARRAY
      IF(NCB.GT.1) THEN
         DO 110 I=1,NT
110      BUFF(I)=0.0
      END IF
...
C : LIST FORMAT
      IF(NCB.EQ.2) THEN
         READ(IU) NLIST
         DO 100 L=1,NLIST
           READ(IU) ICRL,BUFF(ICRL)
100      CONTINUE
 
C : 2-D ARRAY WITH INTEGER ARRAY FOR LAYER CODES
      ELSE IF(NCB.EQ.3 .AND. NLAY.EQ.1) THEN
         READ(IU) (BUFF(I),I=1,NRC)
         READ(IU) (BUFF(I),I=1,NRC)
      ELSE IF(NCB.EQ.3 .AND. NLAY.GT.1) THEN
         READ(IU) (BUFF(I),I=1,NRC)
         READ(IU) (BUFF(I),I=NRC+1,2*NRC)
         DO 115 I=1,NRC
           IF (BUFF(I).EQ.1.0) THEN
             BUFF(I)=BUFF(I+NRC)
             BUFF(I+NRC)=0.0
           ELSE IF(BUFF(I).EQ.2.0) THEN
             BUFF(I)=0.0
           ELSE IF(BUFF(I).GT.2.0) THEN
             ILAY= BUFF(I)
             INEW=I+(ILAY-1)*NRC
             BUFF(INEW)=BUFF(I+NRC)
             BUFF(I)=0.0
             BUFF(I+NRC)=0.0
           END IF
115      CONTINUE
 
C : 2-D ARRAY ASSIGNED TO LAYER 1
      ELSE IF(NCB.EQ.4) THEN
         READ(IU) (BUFF(I),I=1,NRC)
      END IF
...
C***** SUBROUTINE *****
      SUBROUTINE GETBDH(IU,KSTP,KPER,TEXT,NC,NR,NL,NCB,DELT,PERTIM,
     1                  TOTIM,NEWBUD,IEND)
      CHARACTER*(*) TEXT
      IEND=0
      NEWBUD=0
      READ (IU,END=30,ERR=40) KSTP,KPER,TEXT,NC,NR,NL
 
C... NEGATIVE LAYER NUMBER MEANS THIS IS ONE OF THE NEW BUDGET FORMATS,
C    SO NEED TO READ SECONDARY HEADER RECORD
      NCB=1
      IF(NL.LT.0) THEN
         NEWBUD=1
         NL= -NL
         READ(IU,ERR=50) NCB,DELT,PERTIM,TOTIM
      END IF
      RETURN
...
C
     ...
     5IBSTRT(NCOL,NROW,NLAY),BUFF(NCOL,NROW,NLAY)
C
      NOLD=0
