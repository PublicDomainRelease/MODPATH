...
C  CHECK TO SEE IF FILE EXISTS
      IF (EX) GO TO 20
      IF (IBATCH.EQ.0) THEN
      CALL PUTSCR('THE FOLLOWING FILE DOES NOT EXIST:')
      CALL PUTSCR(FNAME)
      CALL PUTSCR('ENTER THE NAME OF AN EXISTING FILE (<CR>=QUIT):')
      CALL GETSTR(FNAME,-1,' ')
      IF (FNAME.EQ.' ') CALL ABORT2
      GO TO 10
      ELSE
...
30    INQUIRE (FILE=FNAME,EXIST=EX)
      IF (EX) THEN
      IF (IBATCH.EQ.0) THEN
      CALL PUTSCR('THE FOLLOWING FILE ALREADY EXISTS:')
      CALL PUTSCR(FNAME)
      CALL PUTSCR('ENTER THE NAME OF A NEW FILE (<CR>=QUIT):')
      CALL GETSTR(FNAME,-1,' ')
      IF (FNAME.EQ.' ') CALL ABORT2
      GO TO 30
      ELSE
...
C
40    IF (IBATCH.EQ.0) THEN
         WRITE(CU,'(I5)') IU
         CALL PUTSCR('ERROR OPENING FILE TO UNIT '//CU)
      END IF
...
C  IF IVER=0 AT THIS POINT, THERE IS SOMETHING WRONG IN THE ENDPOINT FILE
      IF(IVER.EQ.0) THEN
        CALL PUTSCR('INVALID ENDPOINT FILE. STOP.')
        CALL ABORT
      END IF
...
C
      ...
      RETURN
10    CONTINUE
      CALL PUTSCR('No available unit to open file. STOP.')
      CALL ABORT
      END
