...
20    HEAD(J,I,K)=BUFF(NN)
      END IF
c  Stop reading the head file if beyond the desired time step
      if(KPER.GT.IPER .or. (KPER.EQ.IPER .and.KSTP.GT.ISTP)) go to 25
      GO TO 10
25    CONTINUE
...
40    HEAD(J,1,K)=BUFF(NN)
      END IF
c  Stop reading the head file if beyond the desired time step
      if(KPER.GT.IPER .or. (KPER.EQ.IPER .and.KSTP.GT.ISTP)) go to 45
      GO TO 30
45    CONTINUE
      END IF
