...
C     **************************************
C     CONVERT A TEXT STRING TO AN INTEGER
C   
C        STATUS = 1 => IS INTEGER
C        STATUS = 0 => IS NOT AN INTEGER
C     **************************************
      SUBROUTINE TEXT2INTEGER(TEXT,IVAL,IERROR)
      CHARACTER*(*) TEXT
      INTEGER IERROR
      INTEGER IVAL
      IERROR = 0
      IVAL = 0
      READ(TEXT,*,ERR=200) IVAL
      RETURN
200   IERROR = 1
      RETURN
      END
...
C        STATUS = 0 => IS NOT AN INTEGER
C     **************************************
      SUBROUTINE TEXT2REAL(TEXT,RVAL,IERROR)
      CHARACTER*(*) TEXT
      INTEGER IERROR
      REAL RVAL
      IERROR = 0
      RVAL = 0
      READ(TEXT,*,ERR=200) RVAL
      RETURN
200   IERROR = 1
      RETURN
      END
...
C     ------------------------------------------------------------------
   10 FORMAT(A)
      ...
      END
C=======================================================================
      SUBROUTINE ULOG(LOGMESS)
C     ******************************************************************
C     WRITE MESSAGE TO LOG FILE
C     ******************************************************************
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE MPDATA, ONLY:IOLOG
      CHARACTER LOGMESS*(*)
C     ------------------------------------------------------------------
   10 FORMAT(1X,A)

      IF (LOGMESS.NE.' ') THEN
        WRITE(IOLOG,10) LOGMESS
      ENDIF
      RETURN
...
5     RETURN
      ...
      SUBROUTINE FINDREFTIME(KPER,KSTP,FRAC,TREF)
      USE MPDATA, ONLY:SIMTIME,NTSTEPS
      INTEGER KPER,KSTP,ISTEP
      REAL TREF,FRAC
      
      CALL GETGLOBALTIMESTEP(ISTEP,KPER,KSTP)
      IF(FRAC.LE.0.0) THEN
        TREF = SIMTIME(ISTEP-1)
      ELSE IF(FRAC.GE.1.0) THEN
        TREF = SIMTIME(ISTEP)
      ELSE
        TREF = (1.0-FRAC)*SIMTIME(ISTEP-1) + FRAC*SIMTIME(ISTEP)
      END IF
      
      RETURN
      END
      
      SUBROUTINE FINDSTEPFROMTIME(ISTEP,TIME)
      USE MPDATA, ONLY:SIMTIME,NTSTEPS
      INTEGER ::N,ISTEP
      
      ISTEP= 0
      IF(TIME.LT.SIMTIME(0) .OR. TIME.GT.SIMTIME(NTSTEPS)) RETURN
      DO N=1,NTSTEPS
        IF(TIME.GE.SIMTIME(N-1) .AND. TIME.LE.SIMTIME(N)) THEN
          ISTEP=N
          RETURN
        END IF
      END DO
      ...
      USE MPDATA, ONLY:NTOFF
      INTEGER ::KSTP,KPER,IOUT,ISTEP
      ISTEP=0
      ...
      SUBROUTINE GETPERIODANDSTEP(IPER,ISTP,KSTEP,NSTP,NPER,IOUT)
      INTEGER, DIMENSION(NPER) :: NSTP
      INTEGER :: N,K,I,IOUT,KSTEP
...
10    CONTINUE
      CALL USTOP(
     1 'SPECIFIED GLOBAL TIME STEP NUMBER IS OUT OF RANGE. STOP.')
      END
      
      SUBROUTINE SET3DARRAYI(IA,IB,NCOL,NROW,NLAY)
      INTEGER ::NCOL,NROW,NLAY
      INTEGER, DIMENSION(NCOL,NROW,NLAY) ::IA
      INTEGER, DIMENSION(NCOL,NROW,NLAY) ::IB
      IB= IA
      RETURN
      END
      
      SUBROUTINE READFILENAMEFROMLINE(LINE,FNAME)
      CHARACTER(LEN=*) LINE
      CHARACTER(LEN=*) FNAME
