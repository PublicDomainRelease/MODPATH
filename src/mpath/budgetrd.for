...
C         READ(IU,ERR=50) KS,KP,TXTSAV,NC,NR,NLCODE
      END IF
      TEXT=TXTSAV
      WRITE(IO,5) TEXT,KP,KS
...
C         READ(IU,ERR=50) NBTYPE,DELT,PERTIM,TOTIM
         IF(NBTYPE.EQ.5) THEN
            READ(IU,END=50,ERR=1000) NVAL
C            READ(IU,ERR=50) NVAL
            IF(NVAL.GT.1) THEN
               READ(IU,END=50,ERR=1000) (CTMP(I),I=1,NVAL-1)
C               READ(IU,ERR=50) (CTMP(I),I=1,NVAL-1)
               DO 10 I=1,NVAL-1
...
C            READ(IU,ERR=30) DBLBUFF
         ELSE IF(NBTYPE.EQ.2 .OR. NBTYPE.EQ.5) THEN
            IF(NLST.GT.0) THEN
               DO 22 N=1,NLST
               READ(IU,END=30,ERR=50) ICELL,(VALD(I),I=1,NVAL)
C               READ(IU,ERR=30) ICELL,(VALD(I),I=1,NVAL)
22             CONTINUE
            END IF
         ELSE IF(NBTYPE.EQ.4) THEN
            READ(IU,END=30,ERR=50) ((DBLBUFF(J,I,1),J=1,NCOL),I=1,NROW)
C            READ(IU,ERR=30) ((DBLBUFF(J,I,1),J=1,NCOL),I=1,NROW)
         ELSE
            READ(IU,END=30,ERR=50) ((DBLBUFF(J,I,1),J=1,NCOL),I=1,NROW)
            READ(IU,END=30,ERR=50) ((IBUFF(J,I,1),J=1,NCOL),I=1,NROW)
C            READ(IU,ERR=30) ((DBLBUFF(J,I,1),J=1,NCOL),I=1,NROW)
...
C         READ(IU,ERR=50) NBTYPE,DELTD,PERTIMD,TOTIMD
         IF(NBTYPE.EQ.5) THEN
            READ(IU,END=50,ERR=1000) NVAL
C            READ(IU,ERR=50) NVAL
            IF(NVAL.GT.1) THEN
