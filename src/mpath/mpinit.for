...
C***** SUBROUTINE *****
      SUBROUTINE SPACE(LCQX,LCQY,LCQZ,LCPOR,LCIBOU,LCXMAX,LCDX,
...
C
      READ(IU,'(A)') LINE
      ICOL=1
      CALL URWORD(LINE,ICOL,IWSTRT,IWLAST,3,IDUMMY,SIZ,I7,IU)
      IF(SIZ.GT.0.) DMAXSIZ=SIZ
...
5030  FORMAT (' MAXIMUM NUMBER OF PARTICLES IS ',I8)
      WRITE (I7,5040) ISM1
5040  FORMAT (I8,' ELEMENTS IN THE "A" ARRAY')
      RETURN
      END
...
C
      ...
      ELSE IF(STRING.EQ.'TIME-SERIES') THEN
        FNAMS(6)=FNAME
        WRITE(I7,5020) IU,FNAME(1:LFNAME)
      ELSE IF(STRING.EQ.'ADVOBS') THEN
        WRITE(I7,5010) IU,FNAME(1:LFNAME)
        CALL OPNFIL (IU,FNAME,4,I7,IBATCH,1)
      ELSE
